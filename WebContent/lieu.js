...
$(document).on('input',"input[name^='nrb_zones']",function(){
	...
	boucle = 1;
	console.log(idBloc)
	$("#prix_zones_"+idBloc).empty();
	while(boucle <= $(this).val()){
	

	$("#prix_zones_"+idBloc).append("<div class='col-5' id='prix_zone_"+boucle+"'><label for='prix_place_"+boucle+"' class='col-2 col-form-label'>Prix de la zone "+boucle+" </label><div class='col-10'><input class='form-control' type='text' value=' ' name='prix_place_"+boucle+"' id='prix_place_"+boucle+"'></div></div>");
	boucle++;
...
$(document).on('focus',"input[name^='nrb_zones']",function(){
